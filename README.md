...
```text
역할,이름,성별,나이,보호자,본인연락처,보호자연락처,단체티 사이즈,교우관계,특이사항,조장
```
...
기존 참석자의 단체티 사이즈만 일괄 수정할 때는 아래처럼 `이름`, `단체티 사이즈` 열만 있는 CSV / 엑셀 파일을 가져옵니다.
...
- 포함 항목: 이름, 성별, 나이, 보호자연락처, 단체티 사이즈, 조장 여부, 교우관계, 특이사항
...
`출석부` 화면에서 행사 방식에 맞게 출석 항목을 관리합니다.

- 외부 2박 3일 행사처럼 접수 확인만 필요하면 기본 `입소` 항목만 사용합니다.
- 내부 성경학교처럼 여러 날을 운영하면 `1일차`, `2일차`, `3일차` 같은 항목을 추가합니다.
- 화면과 출력물은 접수자가 찾기 쉽도록 이름순을 기본으로 합니다.
- `인쇄 / PDF 저장` 버튼으로 전체 학생 출석부를 이름순으로 출력할 수 있으며, 출력물에는 조, 단체티 사이즈 컬럼과 현장 체크용 빈 체크칸이 표시됩니다.
